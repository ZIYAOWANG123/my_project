...
au_sf_new <- cartogram_cont(au, "weight", itermax = 20, prepare = "adjust")


# Show Distorted Contiguous Cartogram
tm_shape(au_sf_new) + tm_polygons("weight", style = "jenks", legend.show = FALSE) +
  tm_layout(frame = FALSE)
...
# Hierarchical by `State`

```{r}

# au_state <- aec_mapdata %>%
#   group_by(StateAb, geometry) %>%
#   summarise(n = n()) %>% 
#   mutate(num_ele = n) %>% 
#   select(-n) %>% 
#   st_as_sf() %>% 
...
# au_sf_state <- cartogram_cont(au_state, num_ele, itermax = 20, prepare = "adjust")
```
